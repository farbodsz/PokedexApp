...
import com.satsumasoftware.pokedex.adapter.AbilityDexAdapter;
...
public class AbilitiesActivity extends BaseActivity implements FilterListItemVGAdapter.OnFilterItemClickListener {
    ...
    private AbilityDBHelper mDbHelper;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDbHelper = new AbilityDBHelper(this);
        populateList(mDbHelper.getAllAbilities());
    ...
    private void populateList(ArrayList<MiniAbility> items) {
        ...
        Collections.sort(items, new Comparator<MiniAbility>() {
            @Override public int compare(MiniAbility ability1, MiniAbility ability2) {
                if (mSortByName) {
                    return ability1.getAbility().compareTo(ability2.getAbility()); // Ascending
                } else {
                    return ability1.getAbilityId() - ability2.getAbilityId(); // Ascending
                }
            }
        });
    ...
    public void onFilterItemClick(View view, int position, String text, boolean isChecked, View itemView) {
        switch (itemView.getId()) {
            case R.id.filterDrawer_llName_content:
                String nameQuery = "(LOWER(" + AbilityDBHelper.COL_NAME + ") LIKE \"" +
                        text.toLowerCase() + "%\")";
            ...
            case R.id.filterDrawer_llGen_content:
                String genQuery = "(" + AbilityDBHelper.COL_GENERATION + "=\"" +
    ...
    private void updateFilteredList() {
        ...
        String[] columns = new String[] {
                AbilityDBHelper.COL_ID,
                AbilityDBHelper.COL_NAME,
                AbilityDBHelper.COL_GENERATION
        };
        ...
        while (!cursor.isAfterLast()) {
            int id = cursor.getInt(cursor.getColumnIndex(AbilityDBHelper.COL_ID));
            String name = cursor.getString(cursor.getColumnIndex(AbilityDBHelper.COL_NAME));
