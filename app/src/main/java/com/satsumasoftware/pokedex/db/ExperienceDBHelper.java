...
public class ExperienceDBHelper extends SQLiteOpenHelper {
    ...
    public ExperienceDBHelper(Context context) {
    ...
    private void populateDatabase(SQLiteDatabase db) {
        ...
        try {
            db.beginTransaction();
            List<String[]> allRows = parser.parseAll(mContext.getAssets().open(CSVUtils.EXPERIENCE));
            for (String[] line : allRows) {
                ContentValues values = new ContentValues();
                int id = Integer.parseInt(line[0]);
                values.put(COL_GROWTH_RATE_ID, id);
                values.put(COL_LEVEL, Integer.parseInt(line[1]));
                values.put(COL_EXPERIENCE, Integer.parseInt(line[2]));
